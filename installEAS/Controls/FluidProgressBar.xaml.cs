...
/// </summary>
internal partial class FluidProgressBar : IDisposable
{
    ...
    protected void OnDelayChanged(Duration oldDelay, Duration newDelay)
    {
        ...
        if (isActive) StopFluidAnimation();

        UpdateTimelineDelay(newDelay);

        if (isActive) StartFluidAnimation();
    }
    ...
    /// </summary>
    public static readonly DependencyProperty DotWidthProperty = DependencyProperty.Register(nameof(DotWidth), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(4.0, OnDotWidthChanged));
    ...
    {
        if (isStoryboardRunning) RestartStoryboardAnimation();
    }
    ...
    /// </summary>
    public static readonly DependencyProperty DotHeightProperty = DependencyProperty.Register(nameof(DotHeight), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(4.0, OnDotHeightChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty DotRadiusXProperty = DependencyProperty.Register(nameof(DotRadiusX), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(0.0, OnDotRadiusXChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty DotRadiusYProperty = DependencyProperty.Register(nameof(DotRadiusY), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(0.0, OnDotRadiusYChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty KeyFrameAProperty = DependencyProperty.Register(nameof(KeyFrameA), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(0.33, OnKeyFrameAChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty KeyFrameBProperty = DependencyProperty.Register(nameof(KeyFrameB), typeof(double), typeof(FluidProgressBar), new FrameworkPropertyMetadata(0.63, OnKeyFrameBChanged));
    ...
    /// </summary>
    public static readonly DependencyProperty OscillateProperty = DependencyProperty.Register(nameof(Oscillate), typeof(bool), typeof(FluidProgressBar), new FrameworkPropertyMetadata(false, OnOscillateChanged));
    ...
    /// </summary>
    public void StartFluidAnimation()
    {
        if (sb == null || isStoryboardRunning) return;
    ...
    /// Stops the animation
    /// </summary>
    public void StopFluidAnimation()
    {
        if (sb == null || !isStoryboardRunning) return;
    ...
    private void ProcessDoubleAnimationWithKeys(DoubleAnimationUsingKeyFrames dakeys, bool isOpacityAnim = false)
    {
        ...
        for (var i = 0; i < dakeys.KeyFrames.Count; i++)
        {
            ...
            if (!targetMap.ContainsKey(i)) targetMap[i] = new KeyFrameDetails() { KeyFrames = new List<DoubleKeyFrame>() };
    ...
    private void UpdateKeyFrames()
    {
        ...
        if (!(width > 0.0) || Visibility != Visibility.Visible) return;
        var point0 = -10;
    ...
    {
        if (!_keyFrameMap.ContainsKey(key)) return;
        ...
        foreach (var frame in _keyFrameMap[key].KeyFrames)
            switch (frame)
            {
                ...
                case EasingDoubleKeyFrame:
                    frame.SetValue(DoubleKeyFrame.ValueProperty, newValue);
                    break;
            }
    }
    ...
    private void UpdateKeyTime(int key, Duration newDuration)
    {
        ...
        foreach (var frame in _keyFrameMap[key].KeyFrames)
            if (frame is LinearDoubleKeyFrame) frame.SetValue(DoubleKeyFrame.KeyTimeProperty, newKeyTime);
            else if (frame is EasingDoubleKeyFrame) frame.SetValue(DoubleKeyFrame.KeyTimeProperty, newKeyTime);
    }
    ...
    {
        if (!_opKeyFrameMap.ContainsKey(key)) return;

        var newKeyTime = KeyTime.FromTimeSpan(newDuration.TimeSpan);
        _opKeyFrameMap[key].KeyFrameTime = newKeyTime;

        foreach (var frame in _opKeyFrameMap[key].KeyFrames.OfType<DiscreteDoubleKeyFrame>()) frame.SetValue(DoubleKeyFrame.KeyTimeProperty, newKeyTime);
    }

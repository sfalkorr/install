...
public class SprocketControl : Control, IDisposable
{
    ...
    /// </summary>
    protected static void OnAlphaTicksPercentageChanged(double oldAlphaTicksPercentage, double newAlphaTicksPercentage)
    {
    }
    ...
    /// </summary>
    public static readonly DependencyProperty IntervalProperty = DependencyProperty.Register(nameof(Interval), typeof(double), typeof(SprocketControl), new FrameworkPropertyMetadata(DEFAULT_INTERVAL, OnIntervalChanged));
    ...
    {
        if (oldIsIndeterminate == newIsIndeterminate) return;
    ...
    private void CalculateSpokesPoints()
    {
        ...
        for (var i = 0; i < TickCount; i++)
        {
            ...
            if (!IsIndeterminate) angle += _angleIncrement;
            else
                switch (Rotation)
                {
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        for (var i = 0; i < TickCount; i++)
        {
            ...
            if (IsIndeterminate)
            {
                ...
                if (alpha < LowestAlpha) alpha = (byte)LowestAlpha;
            }
        }
    ...
    {
        _dispatcherOperation = Dispatcher.InvokeAsync(() =>
        {
            switch (Rotation)
            {
                case Direction.CLOCKWISE:
                    ...
                    if (_renderStartAngle >= 360) _renderStartAngle -= 360;
                    break;
                case Direction.ANTICLOCKWISE:
                    _renderStartAngle -= _angleIncrement;

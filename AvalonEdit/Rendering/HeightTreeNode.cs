...
internal sealed class HeightTreeNode
{
    ...
    internal void RemoveDirectlyCollapsed(CollapsedLineSection section)
    {
        ...
        if (collapsedSections.Count != 0) return;
        collapsedSections = null;
        totalHeight       = lineNode.TotalHeight;
        if (left != null) totalHeight  += left.totalHeight;
    ...
    {
        return "[HeightTreeNode " + documentLine.LineNumber + " CS=" + GetCollapsedSections(collapsedSections) + " Line.CS=" + GetCollapsedSections(lineNode.collapsedSections) + " Line.Height=" + lineNode.height + " TotalHeight=" + totalHeight + "]";
    }

    private static string GetCollapsedSections(List<CollapsedLineSection> list)
    {
        if (list == null) return "{}";
        return "{" + string.Join(",", list.ConvertAll(cs => cs.ID).ToArray()) + "}";
    }
#endif
}
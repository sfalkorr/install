...
public static class CharRope
{
...
#if DEBUG
        if (length < 0) throw new ArgumentOutOfRangeException(nameof(length), length, "Value must be >= 0");
#endif
    ...
    {
        if (text.Length == 0) return RopeNode<char>.emptyRopeNode;
    ...
    internal static void WriteTo(this RopeNode<char> node, int index, TextWriter output, int count)
    {
        if (node.height == 0)
        {
            if (node.contents == null) node.GetContentNode().WriteTo(index, output, count);
            else output.Write(node.contents, index, count);
        }
        else
        {
